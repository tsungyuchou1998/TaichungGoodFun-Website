...
const render = async () => {
  ...
  app.innerHTML = `${publicHeader(site)}<main>
    ...
    <section class="stories" id="stories"><header class="section-heading"><div><p class="kicker">${escapeHtml(activeRegion.english_name)} STORIES</p><h2><span>${escapeHtml(activeRegion.name)}</span>${escapeHtml(site.journal.heading_suffix)}</h2></div><p>${escapeHtml(activeRegion.journal.intro)}</p></header><div class="story-filter-bar"><label class="district-filter-form" for="district-filter"><span>AREA FILTER</span><select id="district-filter"><option value="">全部行政區</option>${districts.map((district) => `<option value="${escapeHtml(district)}" ${district === activeDistrict ? 'selected' : ''}>${escapeHtml(district)}</option>`).join('')}</select></label></div><div class="story-grid" id="story-grid" aria-live="polite"></div></section>
...
const initializeStories = (allStories, initialDistrict) => {
  ...
  const renderCards = () => {
    grid.innerHTML = stories.length ? stories.map((story, index) => `<article class="story-card"><a class="card-image" href="${pageUrl('pages/story.html', { story: story.slug })}"><img src="${assetUrl(story.cover_image)}" alt="${escapeHtml(story.title)}" loading="lazy"><span class="card-number">${String(index + 1).padStart(2, '0')}</span></a><div class="card-body"><div class="card-meta"><span>${escapeHtml(story.category)}</span><span>${escapeHtml(story.reading_time)}</span></div><h3>${escapeHtml(story.title)}</h3><p>${escapeHtml(story.excerpt)}</p><div class="admin-tags">${(story.administrative_districts || []).map((district) => `<span>${escapeHtml(district)}</span>`).join('')}</div><a class="card-link" href="${pageUrl('pages/story.html', { story: story.slug })}">閱讀遊記 →</a></div></article>`).join('') : '<p class="empty-message">這個行政區目前沒有公開遊記。</p>';
  };
  const show = (index) => {
